...
const isCapitalized = (str) => {
  let returnArr = [];
  // What exactly is meant by the "It" in "It should only match words, not punctuation"?
  // I'm splitting punctuation using '/[,:(). ]/gm', but matching using '/[A-Z]/gm'
  // I'd be happy to re-do this later, but I'm sort of running out of time.
  // But this is kinda beautifully simplistic:

  str.split(/[,:(). ]/gm).map( each => {
    if (/[A-Z]/gm.test(each)) { returnArr.push(each); }
  });
...
const citiesAtoJ = (arr) => {
  ...
  let voidArr = arr.map( city => {
    if (/\b[A-J]/gm.test(city.charAt())) {
      returnArr.push(city);
    }
  });
