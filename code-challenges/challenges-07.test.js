...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const sqrNum = Math.pow(2, arr[i]);
    outputArr.push(sqrNum);
  }
  return outputArr;
};
...
const charCode = (arr) => {
  let outputArr = [];
  arr.map(char => {
    outputArr.push(char.charCodeAt(0));
  });
  return outputArr;
};
...
const extractAbilities = (arr) => {
  let outputArr = [];
  arr.map(ableSlot => {
    outputArr.push(ableSlot.ability.name);
  });
  return outputArr;
};
...
const extractStats = (arr) => {
  let outputArr = [];
  arr.map(eachStat => {
    let attackPow = eachStat.effort + eachStat.baseStat;
    outputArr.push({
