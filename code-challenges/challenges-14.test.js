...
const toTitleCase = (arr) => {
  ...
  arr.map( each => {
    // Grab each character at [0], then upperCase it.
    // Then append the sliced string after the first char.
    returnArr.push(each.charAt(0).toUpperCase() + each.slice(1));
  });
...
let biggerThanLuke = (arr) => {
  ...
  // Should I have grabbed this in some fancy way?
  let lukeMass = 77; // Because it's 77.

  arr.map( eachChar => {
    // Is eachChar.mass greater than lukeMass?
    if (eachChar.mass > lukeMass) {
      // If yes then add them to the array.
      charArr.push(eachChar.name);
    }
  });

  // Do the dash thing to the array...
  for (let i = 0; i < charArr.length; i++) {
    if (charArr.length - 1 !== i) { // Everything but the last one
      outputStr += `${charArr[i]} - `;
    } else { // The last one doesn't get a dash
      outputStr += charArr[i];
    }
  }
...
const sortBy = (property, arr) => {
  if (property === 'name') {
    ...
    arr.sort((isThis, isThat) => {
      return isThis.price - isThat.price;
    });
  }
  console.log(`${property} arr:`, arr);
  return arr;
};
