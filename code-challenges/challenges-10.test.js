...
const createServer = () => {
  // Sneaky... I'm just going to drop this back in here ;)
  ...
  app.get('/favoritefoods', goodFood);
...
const sayHello = (req, res) => {
  res.send('Yo! Have a look around...');
};
const aboutMe = (req, res) => {
  res.send(`
    Oh me? I'm just a guy coding a thing. I have the normal amount of body parts for a human.
    I like things other people like, except when I don't.
    Dogs are cool. They're like people but better in almost every way. 
    Crows are smart. Maybe too smart. They might rule us one day, so be nice to them. And raccoons.
    I think I got off topic a little. That reduce() assignment has cooked my brain into puddin'.
  `);
};
const goodFood = (req, res) => {
  res.send(`
    Here are some of the best foods (in no particular order):
    * Pizza (Like sushi, even when it's bad...)
    * Salad (Best when topped with a healthy dose of not-salad.)
    * Stew (Everybody like stew, right?)
    * Tacos (los auténticos or even barato fast-food ones.)
    * Donkatsu (Korean breaded boneless pork chop. ^.^)
    * Japchae (Made from dangmyeon, the most interesting noodle IMHO.)
    * Takoyaki (https://en.wikipedia.org/wiki/Takoyaki)
    * Sushi (Like pizza, even when it's bad, it's still pretty good.)
    * Curry (Like... all of them. It's stew with attitide.)
    * Tim Horton's coffee (2x, 2x and a dimebag of assorted Tim Bits. Worth the trip to Canada.)
  `);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  filteredArray = input.map( (each) => {
    let tempArr = [];
    let voidArr = [];
    voidArr = each.map( (each) => {
      console.log('each:', each);
      console.log('typof each:', typeof each);
      if (typeof each !== 'number') {
        return;
      } else if (each % 5 !== 0) {
        return;
      } else {
        tempArr.push(Math.pow(2, each));
        return;
      }
    });
