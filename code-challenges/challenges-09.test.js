...
function getCurrentEvents(request, response){
  let returnArr = mapCurrentEvents();
  response.send(returnArr);
}

const mapCurrentEvents = () => {
  let returnObj = [];
  returnObj = currentEvents.news.map( event => {
...
const countNumberOfElements = (arr) => {
  // Since the array starts at 0 we add 1 to our count to compensate.
  // This is probably the wrong way to do this, but it works.
  let returnNum = 1;
  arr = arr.reduce( () => {returnNum += 1;});
  return returnNum;
};
...
const returnNames = (arr) => {
  // This is possibly wrong, but until someone can explain why I would use
  // reduce() before literally anything else, it's going to have to work.
  // This is after an hour of reading documenation. Hmmmph.
  let returnArr = [];
  let tempArr = arr.reduce( (acc, char, idx) => {
    returnArr.push(char.name);
    return acc;
  }, 0);
  return returnArr;
};
...
const reversedString = (str) => {
  ...
  let returnStr = splitStr.reduceRight( (acc, curr) => { return acc + curr;} );
  return returnStr;
};
