...
const templatingWithMustache = () => {
  // Kinda the same gripe here as Cheerio about a lack of documentation, but...
  // ...Mustache is looking like my kind of templating engine. Not too shabby :)
  ...
  for (let i = 0; i < characters.length; i++) {
    let template = `
      ...
      * {{.}}
      ...
      <p> {{ house }} </p>
    `;
    var renderedTemplate = Mustache.render(template, characters[i]);
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].name === character) {
      returnVal = Object.values(arr[i].children) ? true : false;
      break;
    } else {
      returnVal = false;
    }
