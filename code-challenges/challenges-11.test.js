...
// [{ firstName: "Jane", lastName: "Doe" }, { firstName: "James", lastName: "Bond" }]

const toLastNames = people => {
  let returnArr = [];
  // Iter over arr people
  people.map( person => {
    returnArr.push(`${person.firstName} ${person.lastName}`);
  });
  // create new arr over person full name (use map)
  return returnArr;
};
...
const validateEmail = (email) => {
  // Well, if you bash at it long enough... Seriously... This was 2 hours.
  // Serious reverence/pity for all the people that actually know this.
  if (/^([\w]{1,}(?:\.[\w]+){0,1})@((?:[\w]+\.)*\w[\w]*)\.([a-z]{3}?)$/gm.test(email)) {
    return true;
  } else {
    return false;
  }
...
// Yeah... but it works. I hope that was the point. This also took way too long.
const validatePhoneNumber = (phoneNumber) => {
  if(/(^[(]{1}[0-9]{3}[)]{1}[ ]?[0-9]{3}[- ]{1}[0-9]{4})$|(^[0-9]{3}[- ]?[0-9]{3}[- ]{1}[0-9]{4}$)|(^[0-9]{3}[ -]{1}[0-9]{7}$)|(^[0-9]{9,10}$)/.test(phoneNumber)) {
    return true;
  }
};
