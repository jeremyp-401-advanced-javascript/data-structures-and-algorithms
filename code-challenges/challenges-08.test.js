...
function sayHello(request, response){
  // Is adding Express to the npm dependencies part of the challenge?
  // Maybe I was supposed to initially install it globally instead?
  // Either way, this works now.
...
//const forbiddenValues = [1, 2, 3];
//const arr = [1, 2, 3, 4];
// expect = [4];

//const forbiddenValues = ['Demi', 'Gregor', 'Hound'];
//const arr = ['Gary', 'Charlotte', 'Demi', 'Gregor', 'Hound'];
// expect = ['Gary', 'Charlotte'];

const notInFirstArray = (forbiddenValues, arr) => {
  let outputArr = [];
  outputArr = arr.filter(item => {
    // Use indexOf to check for the 'item' in the 'forbiddenValues'
    let thisIdx = forbiddenValues.indexOf(item);
    // If it isn't found (-1) that's great. Add to the 'outputArr'
    return thisIdx === -1;
  });
  return outputArr;
};
