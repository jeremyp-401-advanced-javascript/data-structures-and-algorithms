...
const sortBackwards = (arr) => {
  arr.sort((isThis, isThat) => {return isThat - isThis;});
  return arr;
};
...
// Orig: ['alphabet', 'Zebra', 'Alphabet', 'carrot']
// Sort: ['alphabet','Alphabet', 'carrot']
// Orig: ['alphabet','Alphabet', 'carrot']
// Sort: [ 'Alphabet', 'alphabet', 'carrot']

const alphabetize = (arr) => {
  arr.sort((isThis, isThat) => {
    if (isThis > isThat) { // If this is greater than that
      return 1; // this comes before that
    } else if (isThis < isThat) { // If that is greater than this
      return -1; // that comes before this
    } else if (isThis === isThat) { // If this and that are the same
      return 0; // this and that should be unchanged
    }
  });
  return arr;
};
...
// Orig: ['alphabet', 'Zebra', 'Alphabet', 'carrot']
// Sort: ['Zebra', 'carrot', 'Alphabet', 'alphabet']

const sortByLength = (arr) => {
  arr.sort((isThis, isThat) => {
    if (isThis.length > isThat.length) { // If this is greater length than that
      return 1; // this comes before that
    } else if (isThis.length < isThat.length) { // If that is greater length than this
      return -1; // that comes before this
    } else if (isThis.length === isThat.length) { // If this and that are the same length
      // If this and that are the same see if one is greater
      // Pretty much copy/paste from challenge 3.
      if (isThis > isThat) { // If this is greater than that
        return 1; // this comes before that
      } else if (isThis < isThat) { // If that is greater than this
        return -1; // that comes before this
      ...
      console.log(`Something has gone very, *very* wrong. Hide yo' wife, hide yo' kids.`);
    }
  });
  return arr;
};
...
const alphabetizeBetter = (arr) => {
  // JP: lowerCase the strings first
  // Solution code here...
};
